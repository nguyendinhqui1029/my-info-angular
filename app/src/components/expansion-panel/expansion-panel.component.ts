...
export class ExpandsionPanelComponent implements OnInit {
  ...
  constructor(private ls: LocatorService, private cd: ChangeDetectorRef) {
    this.dynamicComponentService = this.ls.getService<DynamicComponentService>(
    ...
    if (this.index === 0) {
      ...
      if (!this.dataExpandsion.url && this.dataExpandsion.children.length > 0) {
        this.dynamicComponentService.renderComponent(
          this.ls.getComponent(this.dataExpandsion.children[0].url),
          this.containe,
          this.dataExpandsion.children[0]
        );
        this.markActionForTab(this.dataExpandsion.children[0].id);
      } else {
        this.dynamicComponentService.renderComponent(
          this.ls.getComponent(this.dataExpandsion.url),
          this.containe,
          this.dataExpandsion
        );
      }
  ...
  clickExpandsion(event) {
    this.changeIconName();
    if (
      !this.dataExpandsion.children ||
      this.dataExpandsion.children.length === 0
    ) {
      this.markActionForTabParent(this.dataExpandsion.id);
      if (document.querySelector('.active-child')) {
        while (document.querySelector('.active-child')) {
          document
            .querySelector('.active-child')
            .classList.remove('active-child');
        }
      }
  ...
  markActionForTab(id) {
    ...
    document.getElementById(`${this.formatIdContainer(id)}`).classList.add('active-child');
  }
  ...
  markActionForTabParent(id) {
    if (document.querySelector('.active-parent')) {
      while (document.querySelector('.active-parent')) {
        document
          .querySelector('.active-parent')
          .classList.remove('active-parent');
      }
    }
    document.getElementById(`${this.formatIdContainer(id)}`).classList.add('active-parent');
  }
  
  getComponent(object) {
    this.dynamicComponentService.renderComponent(
      this.ls.getComponent(object.url),
      this.containe,
      object
    );
  }
